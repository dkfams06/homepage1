...
- 관리자 UI를 추가했다. 범위와 운영 전환 경계는 `ADMIN_PLAN.md`를 우선 읽는다. `/[locale]/admin` 이하 5개 화면, 네 언어, TanStack Query, 브라우저 로컬 샘플 CRUD이며 **실제 로그인·서버 RBAC·공개 콘텐츠 연동은 아직 없다**.
- 공개 페이지 파일은 `src/app/[locale]/(site)/`로 이동했다. URL은 바뀌지 않았다. 아래 과거 문서의 공개 페이지 경로에는 `(site)/`를 반영해서 찾는다. 관리자에는 공개 사이트 헤더/푸터를 렌더링하지 않는다.
- 사용자는 기능 확장보다 필수 정보 완성을 선택했다. 아래 Phase 2/3은 과거 후보 목록이며 자동으로 전부 구현하지 않는다.
- 안전·사후관리·공간 안내는 별도 페이지/드롭다운 대신 About의 `ClinicEssentials` 한 섹션으로 통합했다. 실제 운영 근거·사진은 미수급 상태다.
- 문의 페이지도 메인·About과 동일한 Contact Us를 사용한다. Home 메뉴는 5개로 유지하고 상담 절차 섹션은 유지한다.
- 온라인 24시간 접수, 1일 내 답변, 무료 상담, 비용 포함, 담당 전문의 직접 집도 등 미확정 약속을 네 언어에서 제거했다. 진료시간·주소·사업자 정보도 확인 전에는 준비 상태다.
- 미확정 전화/소셜/길찾기는 클릭되지 않는다. 전화·소셜은 `src/content/contact.ts`, 길찾기는 `directionsUrl`, 지도는 `mapEmbedUrl`에 검증된 목적지만 등록한다. 전화 표시와 주소·운영 문구는 네 언어 사전에서도 함께 갱신한다.
- 실제 접수 연동과 정식 정책, 병원 자료 반영은 완료가 아니다. `LAUNCH_READINESS.md`에 자료와 완료 기준을 정리했다.
- 로컬 E2E는 기존 라이브러리 경로로 실행 가능하다: `LD_LIBRARY_PATH=/home/eqr6/.local/share/zeus-playwright-libs/usr/lib/x86_64-linux-gnu npm run test:e2e`. 아래 실행 불가 기록은 이전 환경 진단이다.
- 최신 커밋/원격 반영 상태는 `git log -1`과 원격 브랜치로 확인한다. 아래 §3은 Phase 1 커밋 당시의 기록이다.
