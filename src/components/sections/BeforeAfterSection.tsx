    ...
    <Section tone="base">
      <Container>
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:items-center lg:gap-20">
          <Reveal>
            <BeforeAfterCompare
              beforeReady={assetExists(beforeAfter.before.src)}
              afterReady={assetExists(beforeAfter.after.src)}
            />
          </Reveal>

          <div className="flex flex-col gap-8">
            <Reveal className="flex flex-col gap-5">
              <p className="kicker">{beforeAfter.kicker}</p>
            ...
            <Reveal delay={120}>
              <dl className="grid gap-px border border-line bg-line sm:grid-cols-3">
                {[
                  { term: "시술명", value: beforeAfter.procedure },
                  { term: "촬영 시점", value: beforeAfter.timing },
                  { term: "촬영 기준", value: beforeAfter.condition },
                ].map((item) => (
            ...
            <Reveal delay={180} className="flex flex-col gap-5">
              ...
              <CtaLink href={beforeAfter.cta.href} variant="quiet" className="self-start">
